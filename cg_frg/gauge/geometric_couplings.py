...
def compute() -> dict:
    ...
    # The fine-structure constant (COMPUTED): run the framework's geometric g2(M_G) down the SM
    # one-loop RGE to M_Z, combine with the SM hypercharge:
    #   1/alpha(M_Z) = 4 pi (1/g2^2 + 5/(3 g1^2)) at M_Z
    # with g1 the GUT-normalised hypercharge.  The framework's g2 runs
    # exactly (g2(M_Z) = 0.6511 vs 0.6518); the g1 contribution uses
    # the SM value (the CF-4 g1 deviation moves alpha to 133.6).
    b1 = 41.0 / 10.0
    b2 = -19.0 / 6.0
    ...
    v = float(get("v_HIGGS"))
    L_v = math.log(M_G / v)
    inv_g1_v2 = 1.0 / g1 ** 2 + (b1 / (8.0 * math.pi ** 2)) * L_v
    inv_g2_v2 = 1.0 / g2 ** 2 + (b2 / (8.0 * math.pi ** 2)) * L_v
    ...
    L_run = math.log(M_G / MZ_int)
    # Run BOTH geometric couplings down from M_G to M_Z (one-loop SM
    # RGE): g1(M_G)=g2*kappa and g2(M_G) are the framework's own
    # values — no SM coupling enters the computation.
    inv_g1_MZ2 = 1.0 / g1 ** 2 + (b1 / (8.0 * math.pi ** 2)) * L_run
    inv_g2_MZ2 = 1.0 / g2 ** 2 + (b2 / (8.0 * math.pi ** 2)) * L_run
    g1_MZ = 1.0 / math.sqrt(inv_g1_MZ2)
    g2_MZ = 1.0 / math.sqrt(inv_g2_MZ2)
    alpha_inv = 4.0 * math.pi * (1.0 / g2_MZ ** 2 + 5.0 / (3.0 * g1_MZ ** 2))
    pset("alpha_inv_MZ_pred", alpha_inv, provenance="DERIVED",
         role="internal",
         note=f"alpha^-1(M_Z) = {alpha_inv:.1f} (one-loop: the geometric "
              f"g1(M_G)=g2(k_GUT)*kappa run down and g2(M_G) both run "
              f"down the SM RGE; the kappa acts at k_GUT (the isometry-"
              f"breaking onset))")
